...
async function main() {
  ...
  for (let i = 0; i < 6; i++) {
    const transaction = await realEstate.connect(seller).mint(`https://ipfs.io/ipfs/QmUzEir6CtwukLLuuCnJZWEDnmDoB5fZWvLLmck1QzpN6r/${i + 1}.json`)
  ...
  transaction = await escrow.connect(seller).list(5, buyer.address, tokens(10), tokens(5))
  await transaction.wait()
